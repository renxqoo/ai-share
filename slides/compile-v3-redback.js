...
const { renderRedbackSlide, C } = require("./redback-common");
...
pres.title = "AI 是如何在前端开发流程中应用的 - Redback V3";
pres.lang = "zh-CN";
pres.theme = {
...
pres.background = { color: C.white };

for (let i = 1; i <= 18; i += 1) {
  const num = String(i).padStart(2, "0");
  const { slideConfig } = require(`./slide-${num}.js`);
  renderRedbackSlide(pres, slideConfig);
}

async function main() {
  const output = path.join(__dirname, "output", "ai-frontend-sharing-v3-redback-v3.pptx");
